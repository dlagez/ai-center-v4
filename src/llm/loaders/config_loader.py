...
class ConfigLoader:
    ...
    def load_providers(self) -> list[Provider]:
        ...
        return [Provider.from_dict(item) for item in data.get("providers", [])]

    def load_models(self) -> list[ModelProfile]:
        data = self._load_mapping("models")
        return [ModelProfile.from_dict(item) for item in data.get("models", [])]
